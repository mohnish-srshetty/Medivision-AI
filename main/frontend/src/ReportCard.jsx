    ...
    <Card className="shadow-md w-full min-h-screen print:min-h-0" ref={cardRef}>
      ...
      <Tabs defaultValue="summary" className="w-full">
        ...
        <TabsContent value="findings" className="pt-3">
          <CardContent>
            <h4 className="font-medium text-sm text-slate-700 mb-2">Model Predictions</h4>
            <p className="text-xs text-slate-500 mb-3">AI model confidence scores for each condition:</p>
            <ul className="space-y-2">
              {report.findings && report.findings.length ? report.findings.map((finding, index) => {
                // Findings are arrays: [[label, prob], [label, prob], ...]
                const isArray = Array.isArray(finding);
                const label = isArray ? finding[0] : finding;
                const probability = isArray ? finding[1] : null;
                
                return (
                  <li key={index} className="flex items-center justify-between gap-2 p-2 rounded bg-slate-50">
                    ...
                    {probability !== null && (
                      <span className={`text-sm font-semibold ${
                        probability >= 0.85 ? 'text-green-600' : 
                        probability >= 0.50 ? 'text-amber-600' : 
                        'text-slate-500'
                      }`}>
                ...
                <p className="text-sm text-slate-500 italic">No predictions available.</p>
              )}
            </ul>
          </CardContent>
        </TabsContent>
